...
public class SubroutineHierarchy extends JPanel {
	...
	{ 
		...
		//szprychy
        for (int i = 0; i < (N/2); i++ ) {
        	Shape line = new Line2D.Float((float)Math.sin(2*Math.PI/N*i)*2, (float)Math.cos(2*Math.PI/N*i)*2, (float)Math.sin(2*Math.PI/N*(i+4))*2, (float)Math.cos(2*Math.PI/N*(i+4))*2);
        	g2.draw(line);
        }
